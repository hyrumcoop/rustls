...
impl OutboundOpaqueMessage {
    ...
    pub fn copy_to_borrowed<'a>(&self, outgoing_buffer: &'a mut [u8]) -> Option<OutboundOpaqueMessageBorrowed<'a>> {
...
impl PrefixedPayload {
    ...
    pub fn copy_to_borrowed<'a>(&self, outgoing_buffer: &'a mut [u8]) -> Option<PrefixedPayloadBorrowed<'a>> {
        let mut payload = PrefixedPayloadBorrowed::new(outgoing_buffer)?;
...
impl<'a> PrefixedPayloadBorrowed<'a> {
    pub fn new(buffer: &'a mut [u8]) -> Option<Self> {
        match buffer.len() {
            0..HEADER_SIZE => None,
            _ => Some(Self { buffer, payload_len: 0 })
        }
    }
